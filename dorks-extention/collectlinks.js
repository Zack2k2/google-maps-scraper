...
(() => {
  ...
  */
     ...
     */
    getSearchedLinks('div[role="feed"]')

    function getSearchedLinks(css_selector) {
        const data = [...document.querySelectorAll(css_selector)].map(n => n.href);
        return data;
    }
    ...
    */
    ...
    document.addEventListener("mousedown", (event) =>
    {
        ...
        scrollTimeout = setTimeout(
            ()=>{
            google_links=getSearchedLinks('div.Nv2PK a');
            console.log(google_links)
            for (let index = 0; index < google_links.length; index++) {
              sendLinkToLocalServer(google_links[index])
